...
firebase_Stops_UsersQuantity_update = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("stops/{idStopFb}").onUpdate( async (change, context)=>{
      FirebaseFcn.initapp();
...
firebaseToOdoo_Stops_update = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("stops/{idStopFb}").onUpdate( async (change, context)=>{
      FirebaseFcn.initapp();
...
firebaseToOdoo_Routes_update = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("/Route_definition/{idRouteFb}").onUpdate( async (change, context)=>{
      FirebaseFcn.initapp();
      const partnerIds_before = change.before.val();
      const partnerIds_after = change.after.val();
      let borrar = false;
...
firebaseToOdoo_Stops_create = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("/stops/{idStopFb}").onCreate( async (change, context)=>{
      FirebaseFcn.initapp();
...
firebaseToOdoo_Routes_create = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("/Route_definition/{idRouteFb}").onCreate( async (change, context)=>{
      FirebaseFcn.initapp();
...
// Here
firebaseToOdoo_UserTags_update = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("/Data_client/{idUserFb}").onUpdate(async (change, context) => {
      FirebaseFcn.initapp();
...
firebaseToOdoo_Tickets_update = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("/Service_collection/{idTicketFb}").onUpdate(async (change, context) => {
      FirebaseFcn.initapp();
...
firebaseToOdoo_Act_approve = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("/ServiceData_AprovPendant/{idTicketFb}").onDelete(async (change, context) => {
      FirebaseFcn.initapp();
...
Odoo_Contact_createUser = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
...
firebaseToOdoo_CRM_update = functions
    .runWith(
        {secrets: settings.ALL_SECRETS})
    .database.ref("/notRegisteredUsers/{idTicketFb}").onUpdate(async (change, context) => {
      FirebaseFcn.initapp();
...
Odoo_CreateUser = functions
    .runWith({secrets: settings.ALL_SECRETS})
    .https.onRequest( async (request, response)=> {
      FirebaseFcn.initapp();
...
ReadInventory_Odoo = functions
    ...
    .onRequest( async (request, response)=> {
      ...
      // let token = await FirebaseFcn.checkToken(String(request.body.token));
...
ReadZonesMediaSources = functions
    .runWith({secrets: settings.ALL_SECRETS})
    .https.onRequest( async (request, response)=> {
      // return false if fail
      // return list of inventory
...
exports.ClientSym = functions
    .runWith({secrets: settings.ALL_SECRETS})
    .https.onRequest( async (request, response)=> {
      try {
        const firebaseConfig = {
          databaseURL: FirebaseFcn.urldatabase,
          apiKey: process.env.ClientSym_apiKey,
        };
        let app = initializeApp(firebaseConfig);

        let email = process.env.ClientSym_email?process.env.ClientSym_email:"";
        let password = process.env.ClientSym_password?process.env.ClientSym_password:"";
...
exports.CheckTokens = functions.runWith({
  timeoutSeconds: 540,
  secrets: settings.ALL_SECRETS,
}).https.onRequest( async (request, response)=> {
  let tok = "eyJhbGciOiJSUzI1NiIsImtpZCI6ImEyMzhkZDA0Y2JhYTU4MGIzMDRjODgxZTFjMDA4ZWMyOGZiYmFkZGMiLCJ0eXAiOiJKV1QifQ.eyJpc3MiOiJodHR";
  let token = await FirebaseFcn.checkToken(tok);
